...
m = MicMac("/opt/micmac/bin/mm3d","micmac_working_directory","exec/ou3", "FraserBasic", "QuickMac")  # initialisation d'un SFM
#m = Metashape("metashape_working_directory", "exec/ou", 8)
c = CloudCompare("cloudcompare.CloudCompare", "cloudCompare_working_directory", "exec/ou3", "2.11.1", )  # initialisation de PointCloudProcessor
#d = DetectionMetashape()  # initialisation d'un DetecteurMire
d = DetectionCCTag('/opt/CCTag/', 'cctag_working_directory', 'exec/ou3')

# instantiation d'un objet DensityAnalyser construit à partir des modules défini au-dessus
analyser = DensityAnalyser(m, d, c, 'exec/ou3')
